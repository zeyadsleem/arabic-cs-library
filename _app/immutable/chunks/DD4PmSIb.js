const s="hello-algo",n="chapter_computational_complexity",a="تحليل التعقيد",l="space_complexity",p="التعقيد المكاني",c=[{depth:2,id:"المساحة-المرتبطة-بالخوارزمية",text:"المساحة المرتبطة بالخوارزمية"},{depth:2,id:"طريقة-الحساب",text:"طريقة الحساب"},{depth:2,id:"الأنواع-الشائعة",text:"الأنواع الشائعة"},{depth:3,id:"الترتيب-الثابت-o1",text:"الترتيب الثابت $O(1)$"},{depth:3,id:"الترتيب-الخطي-on",text:"الترتيب الخطي $O(n)$"},{depth:3,id:"الترتيب-التربيعي-on2",text:"الترتيب التربيعي $O(n^2)$"},{depth:3,id:"الترتيب-الأسي-o2n",text:"الترتيب الأسي $O(2^n)$"},{depth:3,id:"الترتيب-اللوغاريتمي-olog-n",text:"الترتيب اللوغاريتمي $O(\\log n)$"},{depth:2,id:"المقايضة-بين-الزمن-والمساحة",text:"المقايضة بين الزمن والمساحة"}],t=`<p><u>التعقيد المكاني</u> (space complexity) يقيس اتجاه نمو مساحة الذاكرة التي تشغلها خوارزمية مع تزايد حجم البيانات. وهذا المفهوم يشبه كثيراً التعقيد الزمني، إلا أنه يستبدل «زمن التشغيل» بـ«مساحة الذاكرة المشغولة».</p>
...
<p><img src="/images/hello-algo/chapter_computational_complexity--space_types.png" alt="المساحة المرتبطة بالخوارزمية"></p>
...
<ol>
<li><strong>بناءً على أسوأ بيانات الإدخال</strong>: عندما $n &lt; 10$، يكون التعقيد المكاني $O(1)$؛ لكن عندما $n &gt; 10$، تشغل المصفوفة المهيّأة <code>nums</code> مساحة $O(n)$، لذا يكون التعقيد المكاني في أسوأ حالة $O(n)$.</li>
<li><strong>بناءً على ذروة الذاكرة أثناء تنفيذ الخوارزمية</strong>: فمثلاً، قبل تنفيذ السطر الأخير، يشغل البرنامج مساحة $O(1)$؛ وعند تهيئة المصفوفة <code>nums</code>، يشغل البرنامج مساحة $O(n)$، لذا يكون التعقيد المكاني في أسوأ حالة $O(n)$.</li>
</ol>
<div class="lang-tab">
<p class="lang-tab__label">Go</p>
...
<p>التعقيد الزمني لكل من الدالتين <code>loop()</code> و<code>recur()</code> هو $O(n)$، لكن التعقيد المكاني لكل منهما مختلف.</p>
<ul>
<li>تستدعي الدالة <code>loop()</code> الدالة <code>function()</code> عدد $n$ من المرات في حلقة. وفي كل تكرار، تعود <code>function()</code> وتحرّر مساحة إطار مكدسها، لذا يبقى التعقيد المكاني $O(1)$.</li>
<li>في الدالة التعاودية <code>recur()</code>، توجد $n$ من نسخ <code>recur()</code> غير العائدة في الوقت نفسه أثناء التنفيذ، لذا تشغل مساحة إطارات مكدس $O(n)$.</li>
</ul>
<h2 id="الأنواع-الشائعة">الأنواع الشائعة</h2>
<p>ليكن حجم بيانات الإدخال $n$. يوضح الشكل التالي الأنواع الشائعة للتعقيد المكاني (مرتبة من الأدنى إلى الأعلى).</p>
<p>$$
\\begin{aligned}
...
\\end{aligned}
$$</p>
<p><img src="/images/hello-algo/chapter_computational_complexity--space_complexity_common_types.png" alt="الأنواع الشائعة للتعقيد المكاني"></p>
<h3 id="الترتيب-الثابت-o1">الترتيب الثابت $O(1)$</h3>
<p>الترتيب الثابت شائع في الثوابت والمتغيرات والكائنات التي لا يعتمد عددها على حجم بيانات الإدخال $n$.</p>
<p>ويجدر التنبيه إلى أن الذاكرة التي تشغلها تهيئة المتغيرات أو استدعاء الدوال داخل حلقة تُحرَّر عند الانتقال إلى التكرار التالي، لذا لا تتراكم المساحة، ويبقى التعقيد المكاني $O(1)$:</p>
<div class="lang-tab">
<p class="lang-tab__label">TypeScript</p>
...
<h3 id="الترتيب-الخطي-on">الترتيب الخطي $O(n)$</h3>
<p>الترتيب الخطي شائع في المصفوفات والقوائم المترابطة والمكدسات والطوابير وغيرها، حيث يتناسب عدد العناصر مع $n$:</p>
<div class="lang-tab">
<p class="lang-tab__label">TypeScript</p>
...
<p>وكما يوضح الشكل التالي، فإن عمق التعاود لهذه الدالة هو $n$، أي أن هناك $n$ من دوال <code>linear_recur()</code> غير العائدة موجودة في الوقت نفسه، وتستخدم مساحة إطارات مكدس $O(n)$:</p>
<div class="lang-tab">
<p class="lang-tab__label">TypeScript</p>
...
<p><img src="/images/hello-algo/chapter_computational_complexity--space_complexity_recursive_linear.png" alt="تعقيد مكاني من الترتيب الخطي ناتج عن دالة تعاودية"></p>
<h3 id="الترتيب-التربيعي-on2">الترتيب التربيعي $O(n^2)$</h3>
<p>الترتيب التربيعي شائع في المصفوفات والرسوم البيانية، حيث يرتبط عدد العناصر بـ$n$ ارتباطاً تربيعياً:</p>
<div class="lang-tab">
<p class="lang-tab__label">TypeScript</p>
...
<p>وكما يوضح الشكل التالي، فإن عمق التعاود لهذه الدالة هو $n$، وتُهيَّأ مصفوفة في كل دالة تعاودية بأطوال $n$ و$n-1$ و$\\dots$ و$2$ و$1$، بمتوسط طول $n / 2$، وبذلك تشغل في الإجمال مساحة $O(n^2)$:</p>
<div class="lang-tab">
<p class="lang-tab__label">TypeScript</p>
...
<p><img src="/images/hello-algo/chapter_computational_complexity--space_complexity_recursive_quadratic.png" alt="تعقيد مكاني من الترتيب التربيعي ناتج عن دالة تعاودية"></p>
<h3 id="الترتيب-الأسي-o2n">الترتيب الأسي $O(2^n)$</h3>
<p>الترتيب الأسي شائع في الأشجار الثنائية. تأمل الشكل التالي: شجرة ثنائية كاملة ذات $n$ من المستويات تضم $2^n - 1$ عقدة، وتشغل مساحة $O(2^n)$:</p>
<div class="lang-tab">
<p class="lang-tab__label">Go</p>
...
<p><img src="/images/hello-algo/chapter_computational_complexity--space_complexity_exponential.png" alt="تعقيد مكاني من الترتيب الأسي ناتج عن شجرة ثنائية كاملة"></p>
<h3 id="الترتيب-اللوغاريتمي-olog-n">الترتيب اللوغاريتمي $O(\\log n)$</h3>
<p>الترتيب اللوغاريتمي شائع في خوارزميات التقسيم والتغلب. فمثلاً، في الترتيب بالدمج: بمعطى مصفوفة إدخال طولها $n$، يقسم كل تعاود المصفوفة إلى نصفين من نقطة المنتصف، مكوّناً شجرة تعاود ارتفاعها $\\log n$، وتستخدم مساحة إطارات مكدس $O(\\log n)$.</p>
<p>ومثال آخر هو تحويل عدد إلى نص. فبمعطى عدد صحيح موجب $n$، يكون له $\\lfloor \\log_{10} n \\rfloor + 1$ من الخانات، أي أن طول النص المقابل هو $\\lfloor \\log_{10} n \\rfloor + 1$، لذا يكون التعقيد المكاني $O(\\log_{10} n + 1) = O(\\log n)$.</p>
